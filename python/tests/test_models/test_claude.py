...
class TestClaudeAdapter:
    ...
    def test_haiku_simplifies(self, sample_analysis_system):
        haiku = refrase.adapt(sample_analysis_system, "claude-haiku")
        sonnet = refrase.adapt(sample_analysis_system, "claude-sonnet")
    ...
    def test_role_extracted(self, sample_analysis_system):
        result = refrase.adapt(sample_analysis_system, "claude-sonnet")
        assert "<role>" in result.system
        assert "analyst" in result.system.lower()

    def test_user_unchanged(self, sample_extraction_system, sample_user_prompt):
        result = refrase.adapt(sample_extraction_system, "claude-sonnet", user_prompt=sample_user_prompt)
