 ...
 *
 * Produces byte-identical output to helpers/simplify.ts:simplifyPrompt.
 *
 ...
 *
 * @param params.max_steps - Maximum numbered steps to keep (default: 5).
 */
export function simplify(
  ...
  const maxSteps =
    typeof params.max_steps === "number" ? params.max_steps : 5;
  ...
  for (let line of lines) {
    ...
    // Skip example blocks
    if (
      stripped.toLowerCase().startsWith("example") &&
      stripped.includes(":")
    ) {
      inExampleBlock = true;
      continue;
